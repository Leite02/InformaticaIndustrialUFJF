...
class Cliente:
    ...
    def start(self):
        ...
        """
        self.__tcp = socket.socket(
            socket.AF_INET, socket.SOCK_STREAM
        )  # crio objeto do tipo socket
    ...
    def __method(self):  # implemento o método
        ...
        try:
            ...
            while msg != "\x18":
                ...
                tamanho_da_imagemrecebida_codificado = self.__tcp.recv(
                    1024
                )  # receive , COMANDO BLOQUEANTE fica travado até cliente mandar algo, recebe mensagem do cliente 1024 é tamanho do pacote que quero receber
                tamanho_da_imagemrecebida = int.from_bytes(
                    tamanho_da_imagemrecebida_codificado, "big"
                )
                ...
                while tamanho_recebido < tamanho_da_imagemrecebida:
                    img_recebida_bytes += self.__tcp.recv(1024)
                    tamanho_recebido_codificado = len(img_recebida_bytes).to_bytes(
                        4, "big"
                    )
                    tamanho_recebido = int.from_bytes(
                        tamanho_recebido_codificado, "big"
                    )

                img = cv2.imdecode(
                    np.frombuffer(img_recebida_bytes, np.uint8), cv2.IMREAD_COLOR
                )
