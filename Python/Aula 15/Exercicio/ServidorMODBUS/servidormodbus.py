...
class ServidorMODBUS():
    ...
    def __init__(self, host_ip, port): #passa ip e porta, na qual oferecemos o serviço de dados MODBUS
        ...
        self._server = ModbusServer(host=host_ip,port=port,no_block=True,data_bank=self._db) #cria objeto servidor com todas subrotinas do MODBUS (parecido com objeto da api socket)
       #além disso, adicionamos o no_block = True (já cria servidor MultiThread, atende múltiplos clientes) e passamos o databank criado
    ...
    def run(self):  #método START
        ...
        try:
            ...
            while True: #vai colocar dados randomicos (como se fosse o clp com dados armazenados)
                self._db.set_holding_registers(1000,[random.randrange(int(0.95*400),int(1.05*400))]) #coloca no holding de endereço 1000 um valor random
                print('======================')
                print("Tabela MODBUS")
